...
- Custom tool names are honoured consistently: a renamed tool no longer gets a stray `conversation_id` argument, and a real `get_more_tools` keeps its `context` injection and `$mcp_intent`.
