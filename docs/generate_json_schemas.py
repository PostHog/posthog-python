...
def extract_examples_from_docstring(docstring: str) -> list:
    ...
    examples_section_match = re.search(DOCSTRING_PATTERNS['examples_section'], docstring, re.DOTALL)
    if examples_section_match:
        ...
        code_blocks = re.findall(DOCSTRING_PATTERNS['code_block'], examples_content, re.DOTALL)
...
def extract_details_from_docstring(docstring: str) -> str:
    ...
    details_match = re.search(DOCSTRING_PATTERNS['details_section'], docstring, re.DOTALL)
    if details_match:
...
def analyze_parameter(param: inspect.Parameter, docstring: str = "") -> dict:
    ...
    if docstring:
        ...
        args_section_match = re.search(DOCSTRING_PATTERNS['args_section'], docstring, re.DOTALL)
        if args_section_match:
            args_content = args_section_match.group(1)
            # Look for the parameter description
            param_pattern = DOCSTRING_PATTERNS['param_description'].format(param_name=re.escape(param.name))
            param_match = re.search(param_pattern, args_content, re.MULTILINE | re.DOTALL)
            if param_match:
...
def analyze_function(func, name: str) -> dict:
    ...
    try:
        ...
        result = {
            ...
            "returnType": {
                "id": "return_type",
                "name": get_type_name(sig.return_annotation) if sig.return_annotation != inspect.Signature.empty else DOC_DEFAULTS["return_type_void"]
            }
        }
        if examples:
...
def generate_sdk_documentation():
    ...
    for func_name in dir(posthog):
        ...
        if (func_name not in ['Client', 'Posthog'] and 
            hasattr(func, '__module__') and 
            func.__module__ == 'posthog'):
            try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if no_docs:
            print(f"   • {len(no_docs)} types without documentation: {', '.join(no_docs[:5])}{'...' if len(no_docs) > 5 else ''}")
