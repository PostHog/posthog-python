...
`$mcp_llm_model_source` to `client_metadata`. Otherwise, the SDK adds a required
`llm_model` string to each compatible tool schema and records the agent's answer
