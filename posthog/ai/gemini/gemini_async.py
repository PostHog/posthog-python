...
class AsyncModels:
    """
    Async models interface that mimics genai.Client().models with PostHog tracking.
    """
    ...
    async def generate_content(
        ...
        **kwargs: Any,
        ...
        # Use async wrapper for the sync GenAI SDK call
    ...
    async def generate_content_stream(
        ...
        **kwargs: Any,
        ...
        """
        Generate streaming content using Gemini's API while tracking usage in PostHog.
        ...
        Args:
            ...
            posthog_privacy_mode: Whether to redact sensitive information (overrides client default)
            posthog_groups: Group analytics properties (overrides client default)
            **kwargs: Arguments passed to Gemini's generate_content_stream
        """
    ...
    async def _generate_content_streaming(
        ...
        **kwargs: Any,
        ...
        # Run the sync streaming call in executor
        loop = asyncio.get_event_loop()

        def get_sync_response():
            return self._client.models.generate_content_stream(**kwargs_without_stream)

        response = await loop.run_in_executor(None, get_sync_response)
        ...
        async def async_generator():
            ...
            try:
                # Convert sync iterator to async by running in executor
                ...
                while True:
                    chunk, is_done = await loop.run_in_executor(
                        None, get_next_chunk, iterator
                    )
    ...
    async def _capture_streaming_event(
        ...
        # Use the common capture function - run in executor since it might be sync
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None, capture_streaming_event, self._ph_client, event_data
        )
