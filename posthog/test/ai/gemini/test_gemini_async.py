...
async def test_async_client_basic_generation(
    ...
    """Test the async Client/Models API structure"""
...
async def test_async_vertex_ai_parameters_passed_through(
    ...
    """Test that Vertex AI parameters are properly passed to genai.Client in async mode"""
    mock_google_genai_client.models.generate_content.return_value = mock_gemini_response
    ...
    # Create async client with Vertex AI parameters
    AsyncClient(
        vertexai=True,
...
async def test_async_api_key_mode(mock_client, mock_google_genai_client):
    """Test API key authentication mode in async client"""
...
@pytest.mark.asyncio
async def test_async_tool_use_response(
    mock_client, mock_google_genai_client, mock_gemini_response
):
    """Test that tools defined in config are captured in $ai_tools property in async mode"""
    mock_google_genai_client.models.generate_content.return_value = mock_gemini_response
...
async def test_async_function_calls_in_output_choices(
    ...
    """Test that function calls are properly included in $ai_output_choices in async mode"""
...
@pytest.mark.asyncio
async def test_async_error_handling(mock_client, mock_google_genai_client):
    """Test error handling in async client"""
    # Mock an exception from the underlying client
    mock_google_genai_client.models.generate_content.side_effect = Exception(
        "API Error"
    )
    ...
    assert "API Error" in props["$ai_error"]
    assert props["$ai_http_status"] == 0  # Default for SDK errors


@pytest.mark.asyncio
async def test_async_streaming_error_handling(mock_client, mock_google_genai_client):
    """Test error handling in async streaming"""
    # Mock an exception from the streaming method
    mock_google_genai_client.models.generate_content_stream.side_effect = Exception(
        "Streaming Error"
    )
