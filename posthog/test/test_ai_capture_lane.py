...
class TestCaptureAiEventHelper(unittest.TestCase):
    """`_capture_ai_event` rides the AI lane only when the wrapper opted in."""
    ...
    def test_opted_in_prefers_capture_ai(self):
        client = mock.Mock(spec=["capture", "_capture_ai"])
        _capture_ai_event(
            client, "$ai_generation", _dedicated_ai_endpoint=True, distinct_id="d"
        )
