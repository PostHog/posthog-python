...
def test_resolve_conversation_id_applies_to_every_tool():
    # No tool is exempt, the SDK's own virtual tools included: an exempt
    # send_feedback filed its event under a session of its own, orphaning the
    # agent's complaint from the calls it was complaining about.
...
def test_resolve_conversation_id_uses_supplied_when_mintable_shape():
    ...
    assert resolve_conversation_id(True, {"conversation_id": handle}) == (handle, False)
