...
async def test_first_page_collision_lets_the_real_tool_win():
    ...
    server = make_paged_lowlevel([[_REAL_GET_MORE_TOOLS], [_ECHO_TOOL]])
    client = FakeClient()
    messages = []
    instrument(
        server,
...
async def test_later_page_collision_shadows_the_real_tool():
    ...
    server = make_paged_lowlevel([[_ECHO_TOOL], [_REAL_GET_MORE_TOOLS]])
    client = FakeClient()
    messages = []
    instrument(
        server,
...
async def test_collision_un_shadows_once_the_real_tool_is_dropped():
    ...
    server = make_paged_lowlevel(pages)
    instrument(server, FakeClient(), MCPAnalyticsOptions(report_missing=True))
...
async def test_the_probe_asks_the_host_once_per_virtual_tool_call():
    # The probe asks per call rather than trusting a listing, so a server with
    # per-caller catalogues can't answer one caller from another's listing.
    # Pinning the count keeps that cost visible. Counted inside the host's own
    # handler, which the probe calls directly.
    calls = []
...
async def test_unanswerable_ownership_check_delegates_to_the_host():
    ...
    # handler is failing -- it hands the call to the host. Guessing the other way
    # swallows a real tool of theirs silently, for as long as the handler stays
    # unwell.
...
async def test_a_blocked_name_is_not_also_reported_as_shadowed():
    # The host owns the name on page one *and* a later page. Page one blocked
    # injection, so nothing of PostHog's is advertised: warning that the real
    # tool will not run would send them chasing a bug that isn't there.
    server = make_paged_lowlevel([[_REAL_GET_MORE_TOOLS], [_REAL_GET_MORE_TOOLS]])
    messages = []
    instrument(
        server,
        FakeClient(),
...
async def test_a_duplicate_dropped_name_is_not_reported_as_shadowed():
    ...
    # first page, feedback is dropped. On a later page only the tool we actually
    # injected shadows the host's -- the dropped one cannot.
    server = make_paged_lowlevel([[_ECHO_TOOL], [_REAL_GET_MORE_TOOLS]])
    messages = []
    instrument(
        server,
