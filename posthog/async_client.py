...
from posthog.async_request import (
    APIError,
    batch_post,
    decide,
...
class Client(object):
    ...
    async def get_feature_variants(
        self,
        distinct_id,
        groups=None,
        ...
        resp_data = await self.get_decide(
            distinct_id, groups, person_properties, group_properties, disable_geoip
        )
        return resp_data["featureFlags"]

    async def get_feature_payloads(
        self,
        distinct_id,
        groups=None,
        person_properties=None,
    ...
    async def capture(
        ...
        if send_feature_flags:
            try:
                feature_variants = await self.get_feature_variants(
    ...
    async def _enqueue(self, msg, disable_geoip):
    ...
    async def _compute_flag_locally(
        self,
        feature_flag,
        distinct_id,
        *,
        ...
        if not flag_was_locally_evaluated and not only_evaluate_locally:
            try:
                ...
                if response is None:
                    response = False
                self.log.debug(
                    f"Successfully computed flag remotely: #{key} -> #{response}"
                )
            except Exception as e:
